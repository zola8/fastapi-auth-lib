...
@pytest.mark.asyncio
async def test_create_auth_identity_success(in_memory_async_auth_identity_repo):
    user_id = uuid.uuid4()
    identity_in = _create_auth_identity(user_id=user_id, provider_subject="new_sub")
    created = await in_memory_async_auth_identity_repo.create_auth_identity(identity_in)
...
@pytest.mark.asyncio
async def test_create_generates_sequential_ids(in_memory_async_auth_identity_repo):
    i1 = await in_memory_async_auth_identity_repo.create_auth_identity(
        _create_auth_identity(provider_subject="seq1")
    )
    i2 = await in_memory_async_auth_identity_repo.create_auth_identity(
...
@pytest.mark.asyncio
async def test_create_duplicate_provider_subject_raises(in_memory_async_auth_identity_repo):
    await in_memory_async_auth_identity_repo.create_auth_identity(
...
@pytest.mark.asyncio
async def test_create_duplicate_user_id_raises(in_memory_async_auth_identity_repo):
...
@pytest.mark.asyncio
async def test_get_auth_identity_by_id_success(in_memory_async_auth_identity_repo):
    created = await in_memory_async_auth_identity_repo.create_auth_identity(
        _create_auth_identity(provider_subject="get_by_id")
    )
    fetched = await in_memory_async_auth_identity_repo.get_auth_identity(created.auth_identity_id)
...
@pytest.mark.asyncio
async def test_get_auth_identity_by_id_not_found_raises(in_memory_async_auth_identity_repo):
    with pytest.raises(EntityNotFoundException) as exc_info:
        await in_memory_async_auth_identity_repo.get_auth_identity(99999)

    assert exc_info.value.field == "auth_identity_id"


@pytest.mark.asyncio
async def test_get_auth_identity_by_id_invalid_type_raises(in_memory_async_auth_identity_repo):
    with pytest.raises(EntityNotFoundException):
        await in_memory_async_auth_identity_repo.get_auth_identity(-1)

    with pytest.raises(EntityNotFoundException):
        await in_memory_async_auth_identity_repo.get_auth_identity(0)


@pytest.mark.asyncio
async def test_get_auth_identity_by_user_id_success(in_memory_async_auth_identity_repo):
    user_id = uuid.uuid4()
    created = await in_memory_async_auth_identity_repo.create_auth_identity(
        _create_auth_identity(user_id=user_id, provider_subject="by_uid")
    )
    fetched = await in_memory_async_auth_identity_repo.get_auth_identity_by_user_id(user_id)
...
@pytest.mark.asyncio
async def test_get_auth_identity_by_user_id_not_found_raises(in_memory_async_auth_identity_repo):
    fake_user_id = uuid.uuid4()
    with pytest.raises(EntityNotFoundException) as exc_info:
        await in_memory_async_auth_identity_repo.get_auth_identity_by_user_id(fake_user_id)
...
@pytest.mark.asyncio
async def test_get_auth_identity_by_provider_subject_success(in_memory_async_auth_identity_repo):
    created = await in_memory_async_auth_identity_repo.create_auth_identity(
        _create_auth_identity(provider_subject="by_prov_sub")
    )
    fetched = await in_memory_async_auth_identity_repo.get_auth_identity_by_provider_subject(
        provider=AuthProvider.PASSWORD,
...
@pytest.mark.asyncio
async def test_get_auth_identity_by_provider_subject_not_found_raises(in_memory_async_auth_identity_repo):
    with pytest.raises(EntityNotFoundException) as exc_info:
        await in_memory_async_auth_identity_repo.get_auth_identity_by_provider_subject(
            provider=AuthProvider.PASSWORD,
...
@pytest.mark.asyncio
async def test_update_mutable_fields(in_memory_async_auth_identity_repo):
    created = await in_memory_async_auth_identity_repo.create_auth_identity(
    ...
    updated = await in_memory_async_auth_identity_repo.update_auth_identity(
...
@pytest.mark.asyncio
async def test_update_reindexes_provider_subject(in_memory_async_auth_identity_repo):
    created = await in_memory_async_auth_identity_repo.create_auth_identity(
        _create_auth_identity(provider_subject="old_idx")
    )

    update_identity = _create_auth_identity(provider_subject="new_idx")
    await in_memory_async_auth_identity_repo.update_auth_identity(
        created.auth_identity_id, update_identity
    )
...
@pytest.mark.asyncio
async def test_update_to_existing_provider_subject_raises(in_memory_async_auth_identity_repo):
    await in_memory_async_auth_identity_repo.create_auth_identity(
        _create_auth_identity(provider_subject="taken_sub")
    )
    target = await in_memory_async_auth_identity_repo.create_auth_identity(
...
@pytest.mark.asyncio
async def test_update_same_provider_subject_succeeds(in_memory_async_auth_identity_repo):
...
@pytest.mark.asyncio
async def test_update_nonexistent_raises(in_memory_async_auth_identity_repo):
    with pytest.raises(EntityNotFoundException):
        await in_memory_async_auth_identity_repo.update_auth_identity(
...
@pytest.mark.asyncio
async def test_delete_removes_identity(in_memory_async_auth_identity_repo):
    created = await in_memory_async_auth_identity_repo.create_auth_identity(
        _create_auth_identity(provider_subject="to_delete")
    )
    await in_memory_async_auth_identity_repo.delete_auth_identity(created.auth_identity_id)

    with pytest.raises(EntityNotFoundException):
        await in_memory_async_auth_identity_repo.get_auth_identity(created.auth_identity_id)


@pytest.mark.asyncio
async def test_delete_clears_all_indexes(in_memory_async_auth_identity_repo):
    user_id = uuid.uuid4()
    created = await in_memory_async_auth_identity_repo.create_auth_identity(
    ...
    with pytest.raises(EntityNotFoundException):
        await in_memory_async_auth_identity_repo.get_auth_identity_by_user_id(user_id)
...
@pytest.mark.asyncio
async def test_delete_nonexistent_raises(in_memory_async_auth_identity_repo):
    with pytest.raises(EntityNotFoundException):
        await in_memory_async_auth_identity_repo.delete_auth_identity(99999)


@pytest.mark.asyncio
async def test_delete_then_recreate_same_provider_subject(in_memory_async_auth_identity_repo):
    """After delete, provider_subject index should be cleared allowing reuse."""
    original = await in_memory_async_auth_identity_repo.create_auth_identity(
        _create_auth_identity(provider_subject="reuse_sub")
    )
    await in_memory_async_auth_identity_repo.delete_auth_identity(original.auth_identity_id)

    recreated = await in_memory_async_auth_identity_repo.create_auth_identity(
        _create_auth_identity(provider_subject="reuse_sub")
    )
...
@pytest.mark.asyncio
async def test_delete_then_recreate_same_user_id(in_memory_async_auth_identity_repo):
...
@pytest.mark.asyncio
async def test_returned_identity_is_deep_copy(in_memory_async_auth_identity_repo):
    ...
    refetched = await in_memory_async_auth_identity_repo.get_auth_identity(created.auth_identity_id)
