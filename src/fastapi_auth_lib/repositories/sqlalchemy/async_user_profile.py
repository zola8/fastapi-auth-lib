...
from fastapi_auth_lib.repositories.async_user_profile_interface import IAsyncUserProfileRepository
from fastapi_auth_lib.repositories.db_models.db_user_profile import DBUserProfile


class SQLAlchemyAsyncUserProfileRepository(IAsyncUserProfileRepository):
    """SQLAlchemy implementation of IUserProfileRepository."""

    def __init__(self, session: AsyncSession) -> None:
        self.session = session

    async def create_user(self, user: UserProfile) -> UserProfile:
        await self._assert_email_available(user.email)
    ...
    async def get_user_by_id(self, user_id: uuid.UUID) -> UserProfile:
        db_user = await self.session.get(DBUserProfile, user_id)
